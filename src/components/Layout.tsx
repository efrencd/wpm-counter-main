...
export default function Layout() {
  ...
  const inStudentPanel = pathname.startsWith('/student') && pathname !== '/student';
      ...
      <header className="sticky top-0 z-20 border-b border-slate-800/80 bg-slate-950/85 backdrop-blur">
        <nav className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {inStudentPanel && studentSession && (
              <button className="rounded-lg border border-rose-700/50 px-3 py-2 text-sm font-medium text-rose-300 transition hover:bg-rose-950/40" onClick={handleStudentLogout}>
                Logout alumno
              </button>
            )}
          </div>
